				...
				while ( std::getline (in_file,line) )
					{
						...
						if ((arr[4] == "HTTP") && !(line.find("dummy") != std::string::npos))
							{
...
// Open CLI dialog to get input TXT File
...
// main function begins gere
	...
	std::string question;
	file_name=GetFileName( "Select .TXT file you want to process : " );;
	
	// Append to source file workarround so that last HTTP Meaasge gets processed!!!
	cmd_string+="/C echo ";
	cmd_string+='"';
	...
	question+=file_name;
	question+="  Want to proceed?";
	std::cout << file_name;
	int btn = MessageBox( 0, 
		argc > 1 ? argv[1] : question.c_str() , 
		argc > 2 ? argv[2] : "Selection Verification" , 
		MB_YESNO + MB_ICONQUESTION );
		if ( btn == IDYES ) 
			{
				std::cout << "Ok... Going Forward";
			}
	  	else 
	  		{
	    		MessageBox( 0, "No was selected!!!", "Exiting", 0 );
				return 2;
	  		}
	...
	question="Script COMPLETED!\n";
	question+="Output File: ";
	question+=csv_out_file;
	question+="\n";
	MessageBox( 0, question.c_str(), "Information", 0 );
